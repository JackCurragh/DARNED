...
blstdb = dbpth+"/blast_databases/Hsapiens"
...
def range_based(request):
    ...
    try:
        ...
        return HttpResponse(tmplt.render(RequestContext(request,{'message':'Start, End and/or Flank field(s) are/is either empty or not a number.'})))
...
def geneBased(request):# Remove EST from Hereford
	...
	gene = ''
        if seqtype == "refGene":
	    gene = seqname.upper()
            coors = HSCoordinate.objects.filter(assembly=assembly,gene__gene=seqname.upper()).select_related(depth=2).extra(select={'gstart':'coordinate-1','gend':'coordinate+1'}).order_by('coordinate')
            if len(coors)>0:
                bl = 1
        elif seqtype == "refSeq":
            ...
            if cursor.execute("SELECT name2 from refGene WHERE name='%s' LIMIT 1"%(seqname)):
                ...
                coors = HSCoordinate.objects.filter(assembly=assembly,gene__gene=gene).select_related(depth=2).extra(select={'gstart':'coordinate-1','gend':'coordinate+1'}).order_by('coordinate')
                if len(coors)>0:
                    bl = 1
            cursor.close()
            conn.close()
