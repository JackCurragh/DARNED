...
sttc = '/home/DATA/Anmol/DARNED/staticfiles'
blstdb = "/home/DATA/Anmol/DARNED/blast_databases/Drosophila"
...
#############Bed File Part##########
    ...
    resultFile.close()
###################################
    genesearch = "dm_assembly=%s&dm_chrom=%s&seqtype=%s&exotype=%s&flank=%s&pubid=%s&start=%s&end=%s&gene="%(assembly,chrom,seqtype,exotype,flank,pubid,start,end)
    authorsearch = "dm_assembly=%s&dm_chrom=%s&seqtype=%s&exotype=%s&flank=%s&gene=%s&start=%s&end=%s&pubid="%(assembly,chrom,seqtype,exotype,flank,gene,start,end)
    exonsearch = "dm_assembly=%s&dm_chrom=%s&seqtype=%s&flank=%s&gene=%s&pubid=%s&start=%s&end=%s&exotype="%(assembly,chrom,seqtype,flank,gene,pubid,start,end)
    seqsearch = "dm_assembly=%s&dm_chrom=%s&exotype=%s&flank=%s&gene=%s&pubid=%s&start=%s&end=%s&seqtype="%(assembly,chrom,exotype,flank,gene,pubid,start,end)
...
def geneBased(request):# Remove EST from Hereford
	...
	gene = ''
        if seqtype == "refGene":
	    gene = seqname.upper()
            coors = DMCoordinate.objects.filter(assembly=assembly,gene__gene=seqname.upper())
            if len(coors)>0:
                bl = 1
        elif seqtype == "refSeq":
            ...
            if cursor.execute("SELECT name2 from refGene WHERE name='%s' LIMIT 1"%(seqname)):
                ...
                coors = DMCoordinate.objects.filter(assembly=assembly,gene__gene=gene)
                if len(coors)>0:
                    bl = 1
            cursor.close()
            conn.close()
